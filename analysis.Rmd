...
5種全てが均等な場合です．
わかりやすくするために，合計の出現量が1になるようにしています．
dが0.2で，1/dが種数と同じ5です．
</aside>
...
df_pca |>
  ...
  # --- 2. 垂線の描画 ---
  ggplot2::geom_segment(aes(x = x, y = y, xend = x1, yend = y1), color = "red") +
  ggplot2::geom_segment(aes(x = x, y = y, xend = x2, yend = y2), color = "blue") +
  # --- 3. 点の描画 ---
  ggplot2::geom_point(size = 3) +  # 元の点 (黒)
  ggplot2::geom_point(aes(x1, y1), col = "red", size = 2, shape = 1) +  # PC1への投影点 (赤)
  ggplot2::geom_point(aes(x2, y2), col = "blue", size = 2, shape = 1) +  # PC2への投影点 (青)
    # --- 4. テーマ設定 ---
...
左下から右上への実線をひいていますが，これがxとyの両方を説明をするよい軸です．
PCAでは第1軸といいます．
...
```{r pca-arch, include = FALSE}
val <- rep(c(1,2,3,2,1), 10)
sp <- paste0("sp_", stringr::str_pad(unlist(purrr::map2(1:10, 5:14, `:`)), 2, "left", "0"))
st <- paste0("st_", stringr::str_pad(rep(1:10, each = 5), 2, "left", "0"))
...
```{r pca-arch-plot, include = FALSE}
pca_arch <- 
  tbl_arch |>
  ecan::ordination(o_method = "pca")
ecan::ord_plot(pca_arch)
```
...
任意の距離を使用することができ，環境変数の幅が広いときに使います．

RDAはPCAの環境要因対応版です．
メトリックな距離のみを使うことができ，環境変数の幅が狭いときに追加います．
...
path_stand <- "c:/hoge/huga/data.xlsx"
  # 引数sheetでシート名を番号か文字列で指定
df_st <- openxlsx::read.xlsx(path_stand, sheet = 1)
```
...
diversity_gg <- 
  dune_env_df |>
  ggplot2::ggplot(ggplot2::aes(x = Management, y = i)) + # 1 / d
...
箱ひげ図を描画する`geom_boxplot()`では引数で`outlier.shape = NA`を指定して外れ地を非表示にします．
外れ地を非表示にするのは，`geom_jitter()`による個別の点の表示との重複を避けるためです．
</aside>
...
```{r nmds-plot-st-prep}
ord_nmds_st |>
  ggplot2::ggplot(ggplot2::aes(V1, V2, label = .data[["rowname"]])) +
  ggplot2::geom_text() +
  ggplot2::theme_bw()
```
...
isa_gg <- 
  isa_res |>
   ggplot2::ggplot(ggplot2::aes(x = .data[[group]], y = .data[["ind.val"]],
