...
```{r, echo = FALSE, eval = FALSE}
  ...
  # ソースコード
  # https://github.com/matutosi/ecanvis/blob/main/analysis.Rmd
...
pkg <- "dendextend"; if(!require(pkg, character.only = TRUE)) install.packages(pkg) # 図示で使用
...
- テキストファイル(CSV or TSV)で保存(推奨)
  - 誤変換を防ぐ
  - 再利用が容易
  - `openxlsx::read.xlsx()`，`readr::write_csv()`
- スプレッドシートでの保存
  - 組成, 地点, 種を別シートに入力
  - 文字列として保存(誤変換対策)
  - `openxlsx::write.xlsx()`
...
tibble::tibble(x = x, y = 1, val = val, label = label) |>
  ...
  ggplot2::geom_text(ggplot2::aes(label = label), size = 10, color = "white") + 
...
最小値が1で，最大値が出現種数と同じになるためです．
均等度を重視した指数としては，最小値が0で最大値が1になる1-dがよいでしょう．
</aside>
...
ユークリッド距離は，実践で表しており，直線距離です．
マンハッタン距離は，点線で表しており，碁盤の目のような道路をとおる時の距離です．
...
ggd_wrd <- 
  ...
  ggdendro::ggdendrogram()
cowplot::plot_grid(
  ggd_min, ggd_ave, ggd_wrd, 
  labels = c('最近隣法', '群平均法', 'Ward法'))
```

<aside class="notes">
アメリカでの犯罪傾向について，最近隣法，群平均法，Ward法を比較したのがこの図です．
...
また，日本語フォントで文字化けするときは，『Rによる自動化・効率化レシピ集』の第2章を参考にしてください．
...
cls <- 
  df |>
  # 組成形式に
  ecan::df2table(st = "stand", sp = "species", ab = "cover") |>
  ecan::cluster(
