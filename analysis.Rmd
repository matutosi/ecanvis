---
title: "植生データの解析方法"
author: "松村 俊和"
date: "2025-11-21"
institute: "甲南女子大学"

output:
  revealjs::revealjs_presentation:
    self_contatined: false
    theme: simple
    transition: slide
    css: style.css
---

```{r, echo = FALSE, eval = FALSE}
wd <- "D:/matu/work/todo/ecanvis"; setwd(wd)
rmd <- 'analysis.Rmd'
rmarkdown::render(rmd)
  # https://matutosi.github.io/ecanvis/analysis.html
  # https://matutosi.github.io/ecanvis/analysis.html?print-pdf # PDF印刷用
```

```{r, include = FALSE}
pkg <- "tidyverse"; if(!require(pkg)); install.packages(pkg)
pkg <- "knitr"    ; if(!require(pkg)); install.packages(pkg)
pkg <- "flextable"; if(!require(pkg)); install.packages(pkg)
pkg <- "magrittr" ; if(!require(pkg)); install.packages(pkg)
pkg <- "ecan"     ; if(!require(pkg)); install.packages(pkg)
pkg <- "vegan"    ; if(!require(pkg)); install.packages(pkg) # ecanと同時にインストールされる
pkg <- "ggdendro" ; if(!require(pkg)); install.packages(pkg) # 図示で使用
pkg <- "ggrepel"  ; if(!require(pkg)); install.packages(pkg) # 図示で使用

knitr::opts_chunk$set( # 警告，メッセージ，エラーの非表示
  warning = FALSE,
  message = FALSE,
  error   = FALSE)

source("funs.R")
  # memo：他のオプション
  #                   コード表示  結果  実行
  # eval = FALSE    #     ◯      －    －
  # echo = FALSE    #     －      ◯    ◯
  # include = FALSE #     －      －    ◯
```

# 解説の内容

- 基礎的・一般的な手法を紹介
  - 他にも色々とある
  - 他のもの：参考文献や論文で勉強する
- AIに聞けば結果を教えてくれるかも
  - 基本的な考え方を知るのが重要
  - ブラックボックスではダメ

## 全体像

- 解説編
  - データの取り扱い
  - 解析手法の種類
  - 種多様性
  - クラスター分析
  - 序列化
  - 指標種分析
- 演習編

# データの取り扱い

- tidyな形式で保存
- 地点，組成，種で分けて保存
- 被度の変換???

## tidyな形式

- 1つの変数が1つの列
- 1つの観測が1つの行
- 1つのタイプの観測群が1つの表
- 1つの値が1つのセル
- 詳細は文献を参照<br>
  Hadley, W., 2014, Tidy data. Journal of Statistical Software, 59, 1-23.

## tidy VS messy

"Happy families are all alike; every unhappy family is unhappy in its own way."<br>
— Leo Tolstoy

"Tidy datasets are all alike, but every messy dataset is messy in its own way."<br>
— Hadley Wickham

## Messy data

```{r messy}
tidyr::table2
```

## Tidy data

```{r tidy}
tidyr::table2 |>
  tidyr::pivot_wider(c(country, year), names_from = type, values_from = count)
```

## データの種類

- 地点データ：地点，緯度，経度，標高，日時など
- 組成データ：地点，階層，種名，被度など
- 種データ：種名，科，生活型，機能型など

<aside class="notes">
地点データには，地形，天候などもある
組成データには，植物の状態(実生，開花の有無)などもある
種データには，系統関係などもある
</aside>


## パソコンへの入力

- 1行目は項目名
- 行や列を空けない
- テキストファイル(CSV or TSV)で保存(推奨)
  - 意図しない変換が起こりにくい
  - 容易に使いまわし可能
- スプレッドシートで保存するとき
  - 組成，地点，種を別のシートに入力
  - 勝手に変換されないように文字列として保存

## 量的データと被度

- 在・不在
- Braun-Blanquetの階級値
- 百分率
- その他もある(DAVE参照?)

```{r, include = FALSE}
tbl <- tibble::tribble(
  ~"在・不在" , ~"B-B階級値", ~"百分率" , ~"百分率の代表値", 
  "1"         , "5"         , "75-"     , "87.5"         , 
  "1"         , "4"         , "50-"     , "62.5"         , 
  "1"         , "3"         , "25-"     , "37.5"         , 
  "1"         , "2"         , "10-"     , "17.5"         , 
  "1"         , "1"         , "1-"      ,  "5.0"         , 
  "1"         , "+"         , " -0"     , "0.1"          , 
  "1"         , "r"         , " -0"     , "0.01?"        , 
  "0"         , "-"         , "0"       , "0"            )
caption <- "量的データの種類"
gen_table(tbl, caption)
```

<aside class="notes">
生態調査法(鈴木時夫 p61)
星野(1991)，03479.pdf参照
</aside>

# 解析の種類

- 多様性指数：α，β，γ
- 分類：クラスター分析，TwinSpan，...
- 序列化(Ordination)：PCA，DCA，CCA，RDA，nMDS，...
- 指標種分析(ISA：Indicator Species Analsis)
- その他もいろいろ：Rarefaction Curve, 時系列，GIS，...

# 多様性指数

- α：地点の多様性<br>
  - 1地点 / 単位面積あたりの種数
  - 均等度の考慮の有無
- γ：地域の多様性
- β：地点間の多様性(類似度)
  - β = γ / α
  - β = γ - α

## α多様性

- 代表的な指数
  - 種数(S)
  - Shannonの情報量(H')
  - Simpsonの均等度，1 / d
  - Simpsonの均等度，1 - d
  - S < 1/d < H'
- 個人的見解：1/dが理解しやすい
  - 最小値：1
  - 最大値：S(全種が均等)

## 均等度の考慮

- 用語の意味
  - nij：n地点における種iの地点jでの優占度
  - Nj：地点jの全種の優占度の合計
  - pi：相対優占度算
- 計算方法
  - Nj = Σnij
  - pi = nji / Nj

## 相対優占度

- 地点jで種a,b,c,d,eとも全て0.2のとき
  - Nj = 1.0
  - pa = 0.2 / 1.0
  - 他種も同じ
- 種Aが0.7,他が0.1のとき
  - Nj = 1.0
  - pa = 0.7 / 1.0
  - pb = 0.1 / 1.0

## 計算方法

- H' = -Σ(pi・ln(pi))
- d =  Σ(pi)^2
  - 1 - D
  - 1 / d

## H'の計算

- H' = -( (0.2 * ln(0.2)) * 4)
- H' = -( (0.7 * ln(0.7)) + (0.1 * ln(0.1) * 3)

## dの計算

- d = -( 0.2^2 * 4)
- d = -( 0.7^2 + 0.1^2 * 3)

# クラスター分析

- 概要
  - 地点(種)間の類似度を計算
  - 「近いもの」を結合していく
  - 類似度と結合方法によって結果が異なる
  - 横方向の並びは意味無し
- 結合方法は多種多様(代表的なもの)
  - 最近隣方式
  - 最遠隣方式
  - 重心方式
  - 群平均法<br>
    非加重，加重

## 類似度

- ユークリッド距離
- bray-curtis
- マンハッタン距離(碁盤の目の移動)
- correlation(相関係数)
- QS = 2a / (2a+b+c), Sorensen
- CC = a / (a+b+c), Jaccard

φ = (ad - bc) / ((a+b)(a+c)(b+d)(c+d))

<aside class="notes">
小林(1995)のp26
</aside>

## クラスター分析まとめ

- 個人的見解
  - 地点や種を結びつけていくイメージ
  - 1次元的
  - 非荷重群平均法が多い
  - TwinSpanは他とちょっと違う

# 序列化，Ordination

- 手法は多種多様(代表的なもの)
  - PCA(主成分分析)
  - PCoA
  - DCA：植生以外ではあまり使われない
  - nMDS
  - CCA：環境軽傾度との対応
  - RDA：環境軽傾度との対応
- 類似度との組み合わせで結果が異なる


## PCA


## DCA


## nMDS


## CCA


## 序列化まとめ

- 個人的見解
  - 2次元への位置づけ<br>
  本当は多次元だが，実質的には2次元だけ
  - nMDSが多い

# 指標種分析

- 特定のグループに特徴的に出現する種を抽出
- 量も考慮にいれる
- Dufrene, M. and Legendre, P. 1997. Species assemblages and indicator species: the need for a flexible asymmetrical approach. Ecol. Monogr. 67, 345-366.

## 指標値の計算

- 

# データ分析の実演

- パッケージの準備
- veganパッケージのデータ
- 前処理：かなり重要
- α多様性
- クラスター分析
- 序列化
- 指標種分析

## 参考URL

- https://matutosi.github.io/ecan/ とほぼ同じ内容
- https://matutosi.shinyapps.io/ecanvis/ でも実行可能

## 準備：パッケージのインストール

- tidyverse: プログラムを簡単に
- vegan: 植生データ解析の定番
- ecan: 各種関数を簡単に

```{r install-packages, eval = FALSE}
install.packages("tidyverse")
install.packages("ecan")
install.packages("vegan")    # ecanと同時にインストールされる
install.packages("ggdendro") # 図示で使用
install.packages("ggrepel")  # 図示で使用
```

1回実行すればOK

## 準備：パッケージの呼び出し

```{r, include = FALSE}
library(vegan)
library(ecan)

library(dplyr)
library(tidyr)
library(purrr)
library(tibble)
library(ggplot2)
```

```{r load-library, eval = FALSE}
library(vegan)
library(ecan)
library(tidyverse)
```

## dune.envデータ(地点)

```{r dune-env}
data(dune.env) # 地点
dune_env_df <- 
  tibble::tibble(dune.env) |> # A1, Moisture, Management, Use, Manure
  tibble::rownames_to_column("stand") |> # 地点名を列に変換
  print(n = 8) # 8行のみ
```

## duneデータ(組成)

```{r dune}
data(dune) # 組成
tibble(dune) |> # 0-9の値
  dplyr::select(1:5) |> # 5列のみ表示
  print(n = 8) # 8行のみ
```

## duneデータの変換

```{r dune-df}
dune_df <- 
  dune |>  # 組成表形式からdf形式に変換
  ecan::table2df(st = "stand", sp = "species", ab = "cover")
dune_df
```

## 種データ(ダミー)

```{r sp-dammy}
sp_dummy <- tibble::tibble(
  "species" = colnames(dune), 
  "dummy_1" = stringr::str_sub(colnames(dune), 1, 1), # 1文字目
  "dummy_6" = stringr::str_sub(colnames(dune), 6, 6)) # 6文字目
print(sp_dummy, n = 8)
```

## 結合

```{r join}
df <- 
    dune_df |>                       # 組成
    dplyr::left_join(dune_env_df) |> # 地点
    dplyr::left_join(sp_dummy)       # 種
```

## 結合：基本データの完成!

```{r, echo = FALSE, eval = FALSE}
  # コードの表示幅をもう少し長くできる?
```

```{r joined-show}
print(df, n = 8)
```

## α多様性(おさらい)

- s: 種数
- h: Shannon's H'
- d: Simpson's index (1 - D)
- i: Simpson's inverse (1 / D)

## α多様性の計算

```{r diversity}
div <- ecan::shdi(df)
print(div, n = 8)
```

## α多様性(地点を結合)

```{r diversity-add-st}
dune_env_df <- 
  dplyr::left_join(dune_env_df, div) # 地点を結合
```

## α多様性の平均値

```{r diversity-group}
dune_env_df |>
  dplyr::group_by(Management) |>
  dplyr::summarise_if(is.numeric, mean)
```


## α多様性の図示

```{r diversity-ggplot}
diversity_gg <- 
  dune_env_df |>
  ggplot2::ggplot(aes(x = Management, y = i)) + # 1 / d
  ggplot2::geom_boxplot(outlier.shape = NA) +  # 外れ値非表示
  ggplot2::geom_jitter(height = 0, width = 0.1) + 
  ggplot2::theme_bw() 
```

## α多様性の図示

```{r diversity-ggplot-show}
diversity_gg
```

## クラスター分析

```{r cluster}
cls <- 
  df |>
  ecan::df2table() |> # 組成形式に変換
  ecan::cluster(c_method = "average", d_method = "bray")
```


## クラスター分析の結果

```{r cluster-result}
cls
```

## クラスター分析結果の図示

```{r cluster-plot, eval = FALSE}
library(ggdendro)
ggdendro::ggdendrogram(cls)
```

## クラスター分析結果の図示

```{r cluster-plot-show, echo = FALSE}
  # echo = FALSEでコード非表示
  # include = FALSE # 実行のみで，コード・出力・図は非表示
library(ggdendro)
ggdendro::ggdendrogram(cls)
```

## クラスター分析(図示の準備)

```{r cluster-plot-group-prep}
indiv <- "stand"
group <- "Use"
cls_group <- ecan::cls_add_group(cls, dune_env_df, indiv = indiv, group = group)
```

## クラスター分析(グループ別で図示)


```{r cluster-plot-group}
ggdendro::ggdendrogram(cls_group)
```

## クラスター分析(グループ別で着色)

```{r cluster-plot-group-color}
cls_color <- stats::as.dendrogram(cls_group)
col <- cls_color(cls, dune_env_df, indiv = indiv, group = group)
  # dendextend::labels_colors(cls_color) <- gray(0)
```

## クラスター分析(グループ別で着色)

```{r cluster-plot-group-color-plot}
plot(cls_color)
dendextend::colored_bars(colors = col, cls_color, group, y_shift = 0,  y_scale = 3)
par(new = TRUE)
plot(cls_color)
```

## 序列化：nMDS

```{r nmds}
ord_nmds <- 
  df  |>
  df2table() |>
  ordination(o_method = "nmds")
```

## 序列化：地点の図示の準備

```{r nmds-plot-st-prep-data}
(ord_nmds_st <- ecan::ord_extract_score(ord_nmds, score = "st_scores"))
```
## 序列化：地点の図示

```{r nmds-plot-st-prep}
ord_nmds_st |>
  ggplot2::ggplot(aes(V1, V2, label = .data[["rowname"]])) +
  ggplot2::geom_text() +
  ggplot2::theme_bw()
```


## 序列化：種の図示の準備

ord_add_group()で抽出も実行

```{r nmds-plot-sp-prep}
indiv <- "species"
group <- "dummy_1"
ord_nmds_sp <- ecan::ord_add_group(ord_nmds, score = "sp_scores", df, indiv, group)
```

## 序列化：種の図示用のデータ

```{r nmds-plot-sp-prep-show}
ord_nmds_sp
```

## 序列化：種の図示

```{r nmds-plot-sp-plot}
ord_nmds_sp |>
  ggplot2::ggplot(aes(V1, V2, label = .data[[indiv]])) +
  geom_point(aes(col = .data[[group]]), alpha = 0.4, size = 7) +
  ggplot2::geom_text() +
  ggplot2::theme_bw()
```

## 指標種分析

`row_data = TRUE`でlabdsv::indval()のそのままの結果

```{r indval}
isa_res_raw <- ind_val(df, group = "Management", row_data = TRUE)
```

## 指標種分析の結果：種の出現頻度

```{r indval-raw-res-freq}
isa_res_raw$relfrq
```

## 指標種分析の結果：種の平均出現量

```{r indval-raw-res-abundance}
isa_res_raw$relabu
```

## 指標種分析の結果：指標値

```{r indval-raw-res-indval}
isa_res_raw$indval
```

## 指標種分析の結果：まとめ

```{r indval-raw-res-summary}
  # 最大値のクラス(区分)，その指標値とp値
tibble(
  cls = isa_res_raw$maxcls, 
  ind_val = isa_res_raw$indcls, 
  p = isa_res_raw$pval)
```

## 指標種分析の結果：整理した結果

```{r indval-res}
group <- "Management"
(isa_res <- ind_val(df, group = group))
```

## 指標種分析の結果：

```{r indval-plot}
isa_res |>
   ggplot2::ggplot(aes(x = .data[[group]], y = .data[["ind.val"]],
             size = log(1 / (.data[["p.value"]] * 10)),
             label = .data[["species"]])) + 
    ggplot2::geom_point() + 
    ggrepel::geom_text_repel(aes(size = log(1 / (.data[["p.value"]] * 10), base = 5))) + 
    ggplot2::theme_bw() + 
    ggplot2::theme(legend.position = "none")
```

# 文献

- 原典の論文：細かな計算方法
- 書籍，解説記事：全体的なこと

## 書籍

- 佐々木 雄大・小山 明日香・小柳 知代・古川 拓哉・内田 圭. 2015. 生物群集の構造と多様性の解析. 共立出版. 
- 小林 四郎. 1995. 生物群集の多変量解析. 蒼樹書房. 
  絶版．中古がネット書店に．
- 伊藤 秀三(編). 2003. 群落の組成と構造. 朝倉書店. 
  1977年に出版し，絶版．2003年に復刊し，絶版．
  中古がネット書店に．
- Wildi, O. 2017. Data Analysis in Vegetation Ecology, 3rd ed. Cabi.

## 解説記事

- 加藤 和弘. 1996. 生物群集の多変量解析とその地域環境計画への応用. ランドスケープ研究, 60, 46-55.
- 大垣俊一. 1999. 群集組成の多変量解析. Argonauta, 1, 15-26.
- 大垣俊一. 2008. 多様度と類似度、分類学的新指標. Argonauta. 15, 10-22.
- 土居 秀幸・岡村 寛. 2011. 生物群集解析のための類似度とその応用: Rを使った類似度の算出, グラフ化, 検定. 日本生態学会誌, 61, 3-20. 

## Rで自動化・効率化(宣伝)

- 松村 俊和. 2025. Rによる自動化・効率化レシピ集. 森北出版.
- https://github.com/matutosi/r-auto (サポートページ)
- https://www.amazon.co.jp/dp/4627858310/ (Amazon)

