...
function(input, output) {
  ...
  output$st <- renderUI({ varSelectInput("st", "stand: " ,    data = data_file()) })
  output$sp <- renderUI({ varSelectInput("sp", "species: ",   data = data_file()) })
  output$ab <- renderUI({ varSelectInput("ab", "abandance: ", data = data_file()) })
