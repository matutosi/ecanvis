...
- 2026-09-04 07:03 更新 (このセッション，x280-home)．
  **cluster パネルどうしでも TWINSPAN の群を共有できるようにした** (ユーザ指示，選択肢 a)．
  04:59 に入れた `tw_store` は cluster が書き ordination が読む一方向だったが，
  cluster も他パネルの分を読むようにして，仕組みを対称にした．
  - **狙いは分類法の比較**．`cls_1` で TWINSPAN，`cls_2` で Ward 法を走らせ，
    `cls_2` のカラーバーを `cls_1` の群で塗ると，枝分かれの食い違いが目で見える．
  - `group_df()` で **自分の id を `setdiff()` で除く**．除かないと自分の群が
    `twinspan` と `twinspan_<自分の id>` の二重に出る
    (ordination は publish しないのでこの処理は要らない)．
  - **循環はしない**．`cls_raw()` は `tbl` と入力だけに依存し `group_df()` を見ない．
    他パネルで手法を変えると選択肢が更新されるが，これは意図した動き．
  - テスト 277件 -> 288件 (自分の分の重複と，2パネルの群の区別を検査に入れた)．
...
- **判断待ちは無し** (2026-09-04 時点)．気づいたことが出たらここに足す．
  - 使ってみて選択肢が増えすぎると感じたら，**群の名前を `twinspan_<id>` に
    統一する**案がある (いまは自分の分だけ `twinspan` で不揃い)．
    2026-09-04 に選択肢 b として挙げたもので，採らなかった．
