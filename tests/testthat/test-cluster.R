...
test_that("indiv() follows the item checkbox, shown group or not", {
  skip_if_not_installed("shiny")

    # regression: the body assigned to a local `indiv` inside the if branch
    # only, so with the checkbox off the reactive returned itself.  The branch
    # is gone (the group select is updated by an observer of its own), so the
    # two states of Show group take the same path and are checked together.
  shiny::testServer(clusterSever,
                    args = list(data_in = test_df(), tbl = NULL), {
    for(show in c(FALSE, TRUE)){
      session$setInputs(cls_show_group = show, cls_with_sp = FALSE)
...
test_that("the panel builds the two-way table for TWINSPAN only", {
  ...
  shiny::testServer(clusterSever,
    ...
    expect_equal(colnames(two_way())[1], "stand")
      ...
      # and there is no two-way table for the other methods: the reactive is
      # skipped by req(), it does not fail
    session$setInputs(cls_with_sp = FALSE, cl_c_method = "average")
    expect_error(two_way(), class = "shiny.silent.error")
  })
})
